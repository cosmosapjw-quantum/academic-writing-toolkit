...
> The local Workbench wheel, the Claude Code plugin, and the Codex plugin are
> available now. The ChatGPT App
...
| **Agent plugin (Claude Code, Codex)** | installing the same local skill catalogue in an agent host | the local workspace you authorise | normal project files in your clone |
...
Use this route when you want the 20 AWT skills inside Claude Code without
cloning the repository. From inside Claude Code:
...
Restart Claude Code, then run `/academic-writing-toolkit:read` to confirm the
catalogue loaded. Plugin skills are namespaced by plugin name, so every skill is
`/academic-writing-toolkit:<skill>`.

This installs the skills only. It does not install the Workbench wheel, does not
give the ChatGPT App access to local files, and does not bring the validators,
examples, or templates that the from-source route provides. See
[Claude Code setup](docs/setup-claude-code.md) for both routes side by side.

> **Distribution note.** This repository is a fork. The upstream project is
> [yha9806/academic-writing-toolkit](https://github.com/yha9806/academic-writing-toolkit),
> and the badges, release downloads, and the Codex plugin below are published
> from there. The Claude Code plugin marketplace is published from this fork, so
> its install commands use `cosmosapjw-quantum/academic-writing-toolkit`.
...
make plugin-check       # Claude and Codex plugin metadata, skill sync, bundled helpers
...
├── plugins/                 distributable Claude Code and Codex plugin package
...
- [Plugin publishing checklist (Claude Code and Codex)](docs/plugin-publishing-checklist.md)
