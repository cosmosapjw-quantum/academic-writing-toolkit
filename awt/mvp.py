...
performed by an actual Codex CLI invocation (or an explicitly supplied runner
...
def codex_runner(
    source_text: str, workflow_id: str, manuscript_purpose: str
) -> dict[str, Any]:
...
def analyse_document(
    ...
    *,
    ...
    runtime_instruction_profile["applied_by_default_runner"] = runner is codex_runner
    ...
    with tempfile.TemporaryDirectory(prefix="awt-mvp-source-") as temporary:
        ...
        def execute(text: str, _request: Mapping[str, object]) -> Mapping[str, object]:
            checked = _validate_agent_result(
                runner(context, workflow_id, manuscript_purpose),
                text,
                workflow_id,
                allowed_source_refs,
            )
...
def handler(application: MvpApplication) -> type[BaseHTTPRequestHandler]:
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self) -> None:  # noqa: N802
            ...
            except (MvpError, json.JSONDecodeError, UnicodeDecodeError) as error:
